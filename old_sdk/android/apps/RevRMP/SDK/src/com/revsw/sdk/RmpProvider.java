...
import com.revsw.sdk.RmpClient;
...
public class RmpProvider {
	 ...
	 */
	public RmpResponse rmpWrite(String BASE_URL, RmpRequest rmpRequest, boolean useTcp) {
		...
		try {
			flag = false;
			httpUploadClient.main(BASE_URL, rmpRequest, useTcp);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private HttpObject rmpRead2(HttpObject httpObject) {
		...
		if (httpObject instanceof HttpContent) {
			...
			if (chunk instanceof LastHttpContent) {
				rmpResponse.setTotalTime(currentTime - firstByteTimeStamp);
				//rmpResponse.setTotalTime(currentTime - firstTime);
