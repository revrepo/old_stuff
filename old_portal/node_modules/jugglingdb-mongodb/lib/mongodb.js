...
exports.initialize = function initializeSchema(schema, callback) {
    ...
    //s.url=schema.settings.full_connection_string;

    //s=s+"?"+schema.settings.aditional_params;
    //s.url=schema.settings.full_connection_string;

    if (schema.settings.rs) {
        //s.url=s.url+"?"+schema.settings.aditional_params;
        ...
        if (schema.settings.url) {
            ...
            uris.forEach(function(uri) {
                //s.url=uri+"?"+schema.settings.aditional_params;
                uri+="?"+schema.settings.aditional_params;
        ...
        if (schema.settings.url) {
            schema.settings.url = schema.settings.url.split(',')[0];
    ...
    s.safe = s.safe || false;
    s.journal = s.journal || false;
    s.fsync = s.fsync || false;
    s.w = s.w;
...
function MongoDB(s, schema, callback) {
    ...
    if (s.rs) {
        ...
        //console.log("SET",set);
        server = new mongodb.ReplSetServers(set, {rs_name: s.replicaSet});
    } else {
        server = new mongodb.Server(s.host, s.port, {});
    }

    new mongodb.Db(s.database, server, { safe: s.safe, fsync: s.fsync, w: s.w, journal:s.journal}).open(function (err, client) {
        if (err) {
            console.log("ERR",err)
            throw err;
        } 
