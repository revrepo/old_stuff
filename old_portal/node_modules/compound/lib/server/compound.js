...
function CompoundServer(app, root, opts) {
    ...
    if (app) {
        console.log("Start of  Compound");
        if(revportal.ssl_certificates.is_https) {
            var options = {
                key: fs.readFileSync(revportal.ssl_certificates.key),
                cert: fs.readFileSync(revportal.ssl_certificates.cert),
                ca : fs.readFileSync(revportal.ssl_certificates.ca)
            };
	...
	/**var options = {
  	key: fs.readFileSync('/etc/ssl/revsw/revsw.net.key'),
  	cert: fs.readFileSync('/etc/ssl/revsw/revsw.net.crt'),
	ca : fs.readFileSync('/etc/ssl/revsw/gd_bundle-g2.crt')
	};
